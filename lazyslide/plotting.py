...
ADOBE_SPECTRUM = [
    "#0FB5AE", "#F68511", "#4046CA", "#7326D3",
    "#147AF3", "#72E06A", "#7E84FA", "#DE3D82",
    "#008F5D", "#CB5D00", "#E8C600", "#BCE931",
]
...
class SlideViewer:
    ...
    def add_origin(self, ax=None):
        ...
        origin = (-.02, 1.02)
        # x arrow
        ax.arrow(*origin, .1, 0, **arrow_options)
        ax.text(.1, origin[1], "x", transform=ax.transAxes, ha="left", va="center")
        # y arrow
        ax.arrow(*origin, 0, -.1, **arrow_options)
        ax.text(origin[0], 1 - .1, "y", transform=ax.transAxes, ha="center", va="top")

    def add_tiles(self, tiles, tile_ops, value=None, cmap=None, norm=None,
                  palette=None, alpha=None, ax=None, title=None):
        if ax is None:
            ax = self.ax
        if title is not None:
            ax.set_title(title)
        ...
        if value is None:
            ...
            if isnumeric:
                ...
                ax.pcolorfast(v_arr, norm=norm, cmap=cmap, alpha=alpha)
                cat_legend(labels=kinds, colors=[kinds_colors[k] for k in kinds],
                           ax=ax, loc="out right center", title=title)

    def add_contours_holes(self, contours, holes,
                           contour_color="green",
                           hole_color="red",
                           linewidth=1,
                           alpha=None,
        ...
        for contour in contours:
            ax.fill(contour[:, 0] * ratio, contour[:, 1] * ratio,
                    color=contour_color, alpha=alpha, lw=linewidth)
        for hole in holes:
            ax.fill(hole[:, 0] * ratio, hole[:, 1] * ratio,
                    color=hole_color, alpha=alpha, lw=linewidth)
